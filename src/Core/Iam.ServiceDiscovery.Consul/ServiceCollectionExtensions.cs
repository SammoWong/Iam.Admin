...
namespace Iam.ServiceDiscovery.Consul
{
    public static class ServiceCollectionExtensions
    {
        ...
        public static void RegisterToConsul(this IApplicationBuilder app, ConsulConfig config)
        {
            ...
            var serviceUri = GetServiceAddressInternal(app, config);
            var serviceId = serviceRegistry.GetServiceId(config.ServiceName, serviceUri.Host, serviceUri.Port);

            lifetime.ApplicationStarted.Register(async () =>
            {
                await serviceRegistry.RegisterServiceAsync(config.ServiceName, config.ServiceUrl, config.HealthCheckUrl, config.ServiceTags);
            });
